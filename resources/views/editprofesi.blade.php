    ...
    <section class="bg-white">
        <div class="mx-auto max-w-screen-xl lg:py-8">
            <div
                ...
                @foreach($editpro as $epro)
                    <form action="/updateprofesi" method="post">
                        {{ csrf_field() }}
                        <input type="hidden" name="id" value="{{ $epro->id }}">

                        <div class="mb-5">
                            <label for="nama_profesi"
                                class="block mb-2 mt-4 text-sm font-medium text-gray-900 dark:text-white">Nama
                                Profesi</label>
                            <input type="text" id="nama_profesi" name="nama_profesi" value="{{ $epro->nama_profesi }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 
                                        focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 
                                        dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        </div>

                        <label for="deskripsi_profesi" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            Deskripsi Profesi
                        </label>
                        <textarea id="deskripsi_profesi" name="deskripsi_profesi" rows="4"
                            ...
                            placeholder="Tulis deskripsi profesi...">{{ $epro->deskripsi_profesi }}</textarea>
